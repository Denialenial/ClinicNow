...
export default function Profile() {
  ...
  const handleSave = async () => {
    if (!profile.name.trim()) {
      Alert.alert(t('errors.error'), t('profile.nameRequired'));
      return;
    }
    ...
    >
      ...
      <View style={styles.header}>
        <Avatar
          ...
          size={100}
        />
        <Text style={[styles.userName, { color: colors.text }]}>{profile.name || t('common.patient')}</Text>
      </View>

      {/* Personal Information Section */}
      <View style={[styles.section, { backgroundColor: colors.card }]}>
        <SectionHeader title={t('profile.personalInformation')} onEdit={handleEdit} editing={editing} />
        ...
        <ProfileField
          ...
          value={profile.dateOfBirth || t('profile.notSet')}
          editable={false}
        />
      </View>
